...
// @name         AutoIndex
// @version      0.0.3
// @description  AutoIndex Image Helper
...
// @include      *://*
// @supportURL   https://github.com/gausszhou/user-scripts/tree/master/packages/autoindex
// @homepageURL  https://github.com/gausszhou/user-scripts/tree/master/packages/autoindex
// ==/UserScript==
...
// Module
___CSS_LOADER_EXPORT___.push([module.id, ".autoindex{display:flex;flex-wrap:wrap}.autoindex img{width:200px}.autoindex .image-box{border:1px solid #333}.autoindex .image-name{text-align:center}\n", ""]);
// Exports
...
UI.prototype.init = function () {
  console.log("欢迎使用AutoIndex");
};

UI.prototype.mounted = function () {
  this.body = document.body;
  let flag = dq("body > img");
  if (flag) {
    console.log("处理中");
    this.process();
  }
...
UI.prototype.process = function () {
  ...
  if (imgList.length) {
    ...
    Array.from(imgList).forEach((item) => {
      arr.push({
        br: item.previousSibling,
        text: item.previousSibling.previousSibling,
        img: item,
        num: parseInt(item.previousSibling.previousSibling.data.match(/\d+/)[0])
      });
    });
    ...
    arr.forEach((item) => {
      ...
      p.innerHTML = item.text.data;

      container.appendChild(p);
      let imgBox = document.createElement("div");
      imgBox.className = "image-box";
      imgBox.appendChild(item.img);
      container.appendChild(imgBox);
      this.body.appendChild(container);
    });
    this.body.classList.add("autoindex");
  }
};
