...
const simpStr = charPYStr();
const tradStr = ftPYStr();

const tradMapSimp = {};

Array.from({ length: simpStr.length }).forEach((no, index) => {
  const tradChar = tradStr[index];
  const simpChar = simpStr[index];
...
const reader_dq = document.querySelector.bind(document);
const dqs = (selector) => {
...
function onLoad() {
  if (location.hostname === "book.qidian.com") {
    const qidian = new components_qidian();
    qidian.mounted();
  } else {
    const reader = new components_reader();
    reader.mounted();
    reader.judge();
  }
